...
export function PurchaseForm() {
  ...
  useEffect(() => {
    if (state.ok && handledRef.current !== state) {
      ...
      setStore(null);
      setFormKey((k) => k + 1);
      success(state.message ?? "Purchase logged");
  ...
  return (
    <form key={formKey} action={formAction} className="space-y-5">
        ...
        >
          {(p) => (
            <Input
              ...
              required
              placeholder="0.00"
              invalid={Boolean(state.errors?.price_paid_myr?.[0])}
            />
          )}
        </Field>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label="Brand">
          {(p) => <Input {...p} name="brand" placeholder="optional" />}
        </Field>
        <Field label="Origin">
          {(p) => (
            <Input
      ...
      <Field label="Notes">
        {(p) => <Input {...p} name="notes" placeholder="optional" />}
      </Field>

      <EvidenceUpload />

      <Button type="submit" size="lg" block variant="accent" disabled={pending}>

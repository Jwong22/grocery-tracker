...
import { submitPurchase, type PurchaseFormState } from "./actions";

const initialState: PurchaseFormState = { ok: false };
...
export function PurchaseForm() {
  ...
  const handledRef = useRef<PurchaseFormState | null>(null);
  ...
  const productCreate = useCallback(
    (name: string) => createProductAction(name),
    [],
  );
  const storeCreate = useCallback((name: string) => createStoreAction(name), []);
    ...
    <form key={formKey} action={formAction} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field
          ...
          inputMode="decimal"
          required
          error={state.errors?.price_paid_myr?.[0]}
        />
        <Field
          label="Quantity"
          name="qty"
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label="Brand" name="brand" placeholder="optional" />
        <Field
          label="Origin"
          name="origin_country"
      ...
      <button
        type="submit"
        disabled={pending}
        className="w-full h-12 rounded-full bg-amber-600 text-white font-medium hover:bg-amber-700 disabled:opacity-60"
      >
  ...
  return (
    <label className="block space-y-1">
      ...
      <input
        name={name}
        {...rest}
        className="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500"
      />
